...
| [setup-workflow](setup-workflow.md) | proposed | The orchestration layer that turns first-run into ONE guided command — sequencing init → pre-bootstrap → onboarding → bootstrap → post-heal → ready, with skip and resume — so the user never has to remember separate commands. | 2026-06-29 |
