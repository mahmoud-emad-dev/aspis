...
- **FR-007**: All universal denies MUST be present in every agent's permission surface: `git commit*` denied except committer (R-004), `git push*` denied for all agents (R-008), `webfetch` denied except system-lead and research-lead, and `websearch` denied except research-lead (system-lead is local/deterministic — it may fetch a known doc but performs no open-ended web search).
...
### Session 2026-06-27 (Build Phase)
- **Skills count**: SPEC says "30+ missing skills" (FR-023, SC-004). Actual T-31 inventory count is 25 (13 P0, 7 P1, 5 P2). The 38 existing skills + 25 missing = 63 total referenced. The "30+" language in the SPEC overstates the actual gap; 25 is the accurate count from cross-referencing all 11 agent frontmatters + 8 lead ref specs against the existing catalog. The SPEC is not being edited to avoid scope creep — this clarification suffices.
- **System-lead websearch**: FR-007 says `webfetch`/`websearch` denied except system-lead and research-lead. However, per T-23 build notes, the catalog was intentionally aligned to the reference spec, which sets system-lead `websearch: deny`. The reference spec (system-lead §4 and §12) restricts system-lead to deterministic scripts and local-only tools; external web access is research-lead's exclusive domain. FR-007 is the side that was incorrect — the catalog and reference spec agree on `deny`. **Resolved:** FR-007 updated to "`websearch` denied except research-lead"; system-lead keeps `webfetch: allow` (fetch a known doc) but `websearch: deny` (no open-ended web search — that injection surface belongs to research-lead alone).
- **Planning-lead future subagents**: 7 future L3 subagents (`clarify`, `task-decomposer`, `idea-capture`, `prd-writer`, `constitution-checker`, `scope-estimator`, `research-request-writer`) are listed in planning-lead's `delegates` field. These are intentional references for F-017 (subagent roster). Per build notes: "Subagent roster is OUT of scope → F-017. The 'future subagents' named in the specs are NOT built here — leave them as references." The catalog file carries a comment marking them as future.
- **bootstrap.md**: A 12th catalog agent file exists at `src/aspis/data/catalog/agents/bootstrap.md`. It is outside F-016 scope (a one-time project onboarding agent, not part of the 11-agent system). It is missing `delegates` and `runtimes` fields but self-deletes post-onboarding. Tracked as follow-up.
- **Skills deferred**: 25 missing skills are specified in T-31 inventory with purpose, owning agent, and priority. Building the actual SKILL.md files is a follow-up feature. The inventory serves as the specification; implementation is out of scope for F-016.
- **Claude Code adapter**: The permission-block preservation is specified in T-37 (cross-runtime parity spec) with 5 acceptance criteria. The actual adapter fix is a follow-up implementation task, not part of F-016's specification scope.
- **CLI verbs**: All 6 missing CLI verbs are specified (T-36) with purpose, priority, and interface signatures. Building them is a follow-up. The `aspis validate-runtime` catalog structural check — needed for T-30 — was performed manually since the CLI does not yet exist.
