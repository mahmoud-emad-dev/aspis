...
1. Orient from the generated index — `.aspis/index/FILE_REGISTRY.yaml` (where files
   are) and `.aspis/index/CODE_MAP.md` (each file's skeleton + imports). Most
...
2. For a fresh or narrower skeleton, run the code-map tool on just the part you
   need: `python .aspis/scripts/context/build_code_map.py --scope <path>` — it
...
- Read-only on product code: never edit or write source; the only commands you run
  are the deterministic context tools under `.aspis/scripts/context/`.
