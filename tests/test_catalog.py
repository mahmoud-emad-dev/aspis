...
def _tier(runtime: str, tier: str) -> str:
    """The model a tier resolves to, read from the live tier map (not hardcoded).

    Keeps these assertions about *which tier* an agent is on, so editing the
    model behind a tier in ``models.yaml`` never breaks them.
    """
    return resources.model_map(runtime)[tier]
...
def test_project_lead_renders_for_claude(tmp_path) -> None:
    ...
    assert fm["model"] == _tier("claude", "standard")  # standard tier resolved
...
def test_system_lead_is_a_deep_authoring_primary(tmp_path) -> None:
    ...
    fm = _frontmatter(text)
    assert fm["model"] == _tier("claude", "deep")  # deep tier — authoring/governance
...
def test_project_explorer_is_a_cheap_readonly_subagent(tmp_path) -> None:
    ...
    fm = _frontmatter(text)
    assert fm["model"] == _tier("claude", "cheap")  # cheap tier
...
def test_planning_lead_is_a_deep_planning_subagent(tmp_path) -> None:
    ...
    fm = _frontmatter(text)
    assert fm["model"] == _tier("claude", "deep")  # deep tier — planning is reasoning-heavy
...
def test_modes_config_ships_and_parses(tmp_path) -> None:
    ...
    modes_file = tmp_path / ".aspis" / "config" / "policy" / "modes.yaml"
    assert modes_file.is_file()  # the `config` kind lands in the brain (policy tier)
...
def test_reviewer_is_a_deep_readonly_authority(tmp_path) -> None:
    ...
    fm = _frontmatter(text)
    assert fm["model"] == _tier("claude", "deep")  # deep tier — quality judgment
